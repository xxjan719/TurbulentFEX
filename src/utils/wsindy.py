"""
Weak SINDy (WSINDy) for polynomial ODE discovery.

Adapted from PyWSINDy-style reference implementations; used for MC triad comparisons.
"""
import itertools
import operator
...
class WSINDy:
    ...
    Inputs:
      ...
      scale_theta: normalize columns of theta. Ex: scale_theta = 0 means no normalization,
        scale_theta = 2 means l2 normalization.
    ...
    def __init__(
        ...
        self.polys = polys
        self.time_polys = [] if time_polys is None else list(time_polys)
        self.trigs = trigs
    ...
    def getWsindyAdaptive(self, xobs, tobs, r_whm=30, s=16, K=120, p=2, tau_p=16):
        ...
        Theta_0, tags, M_diag = self.buildTheta(xobs, tobs)
    ...
    def rhs(self, t, x):
        """Vector field dx/dt at scalar time ``t`` and state ``x`` (length n)."""
        ...
        for row in range(rows):
            x_part_cols = min(cols, x_dim)
            for col in range(x_part_cols):
                term[row] = term[row] * x[col] ** self.tags[row, col]
            if cols > x_dim:
                t_exp = self.tags[row, x_dim]
                term[row] = term[row] * (tf ** t_exp)
    ...
    def simulate(
        ...
        if max_step is None and t_eval.size > 1:
            dts = np.diff(t_eval)
            max_step = float(np.min(dts[np.isfinite(dts) & (dts > 0)]))
    ...
    def Adaptive_Grid(self, t, xobs, params=None):
        ...
        Vp = scipy.sparse.diags(
            Vp_diags.T, np.arange(0, index_gap + 1), (M - index_gap, M)
        )
    ...
    def VVp_build_adaptive_whm(self, t, centers, r_whm, param=None):
        ...
        for k in range(1, M):
            ...
            if a > 0 and b_temp < len(t):
                ...
                p, a, b = self.test_fcn_param(r_whm, t[int(centers[k] - 1)], t)
                a = int(a)
                b = int(b)
                if b - a < 10:
                    center = (a + b) / 2
    ...
    def buildTheta(self, xobs, tobs=None):
        theta_0, tags = self.poolDatagen(xobs, tobs)
        if self.scale_theta > 0:
    ...
    def poolDatagen(self, xobs, tobs=None):
        n, d = xobs.shape
        if len(self.polys) != 0:
            P = self.polys[-1]
        else:
        ...
        def f(t, x):
            return np.prod(np.power(list(t), list(x)))
        ...
        for i in range(len(self.trigs)):
            trig_inds = np.array(
                [-self.trigs[i] * 1j * np.ones(d), self.trigs[i] * 1j * np.ones(d)]
            )
        ...
        # Optional explicit time-basis terms: t, t^2, ... (no x-t cross terms).
        if len(self.time_polys) != 0:
            if tobs is None:
                raise ValueError("time_polys requires tobs in buildTheta/poolDatagen.")
            t_arr = np.asarray(tobs).reshape(-1)
            if t_arr.size != n:
                raise ValueError(
                    f"tobs length ({t_arr.size}) must match xobs rows ({n}) when using time_polys."
                )
            tags_aug = np.hstack([tags, np.zeros((tags.shape[0], 1), dtype=tags.dtype)])
            theta_aug = theta_0
            time_tags = []
            for p in self.time_polys:
                p_int = int(p)
                if p_int < 1:
                    continue
                new_column = (t_arr ** p_int).reshape(-1, 1)
                theta_aug = np.hstack([theta_aug, new_column])
                row = np.zeros((1, d + 1))
                row[0, -1] = p_int
                time_tags.append(row)
            if time_tags:
                tags = np.vstack([tags_aug] + time_tags)
                theta_0 = theta_aug
            else:
                tags = tags_aug
        return theta_0, tags
            ...
            def g(t, t1, tk):
                return (
                    ...
                    * (
                    ...
                    + (p > 0)
                    * (q < 0)
                    * np.sin(p * np.pi / (tk - t1) * (t - t1))
                    + (p == -1) * (q == -1)
                ) / (np.abs(normalize((q * t1 + p * tk) / (p + q), t1, tk)))
